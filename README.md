...
During a Redis outage, affected instances disconnect sockets, reject new connections/joins, and return 503 for presence and failed authorization propagation. Event delivery is best-effort: REST stays authoritative and clients reload state after reconnect. Connection/subscription health is probed every two seconds; recovery reuses the existing listeners and restores subscriptions. Every five seconds, instances also recheck connected users, sessions, and workspace memberships against MongoDB to catch missed revocations. A stalled reconciliation fails closed after five additional seconds. A database mutation may already have persisted when coordination returns 503; do not treat that response as a rollback.
