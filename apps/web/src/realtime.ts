...
const recoverCookieSession = (): Promise<SessionRecovery> => {
  const refresh = async (): Promise<SessionRecovery> => {
    ...
    const response = await fetch(`${socketUrl}/api/auth/refresh`, {
      method: 'POST',
      credentials: 'include',
    });
  ...
  recovery ??= (async () => {
    if (globalThis.navigator?.locks)
      return await navigator.locks.request('flowryn-session-refresh', refresh);
...
export const bindRealtime = (
  ...
  const refresh = () => {
    for (const key of [
      ...
      void queryClient.invalidateQueries({
        ...
        refetchType: key === 'tasks' && queryClient.isMutating() > 0 ? 'none' : 'active',
      });
    }
  };
  const join = (event: 'workspace:join' | 'project:join' | 'incident:join', id: string) =>
    new Promise<boolean>((resolve) => {
      socket
        .timeout(5000)
        .emit(event, id, (error: Error | null, result?: SocketAcknowledgement) =>
          resolve(!error && result?.ok === true),
        );
    });
  const onConnect = () => {
    clearTimeout(retry);
    ...
    void (async () => {
      if (!(await join('workspace:join', workspaceId)))
        throw new Error('Workspace access unavailable');
      if (disposed || current !== generation) return;
      if (projectId && !(await join('project:join', projectId)))
        throw new Error('Project access unavailable');
      if (disposed || current !== generation) return;
      if (incidentId && !(await join('incident:join', incidentId)))
        throw new Error('Incident access unavailable');
      if (disposed || current !== generation) return;
      onState('connected');
      refresh();
    })().catch(() => {
      if (!disposed && current === generation) onState('error');
    });
  };
  const terminal = () => {
  ...
  const recoverSession = () => {
    ...
    void recoverCookieSession()
      ...
      .catch(() => {
        if (!disposed && cycle === generation) retryTransport();
      });
  };
  ...
  const onEvent = (message: { eventId?: string; workspaceId?: string }) => {
    if (message.workspaceId !== workspaceId || !message.eventId || seen.has(message.eventId))
      return;
