...
export const getPresence = (workspaceId: string) => [...new Set(
  [...(io?.sockets.sockets.values() ?? [])].filter((socket) => socket.rooms.has(workspaceRoom(workspaceId))).map((socket) => socket.data.userId as string),
)];
...
export const createRealtimeGateway = (server: HttpServer, allowedOrigins: string[]) => {
  ...
  const announcePresence = (workspaceId: string, userId: string) => {
    publishRealtimeEvent({ type: 'presence.updated', workspaceId, actorId: userId, payload: { userId, online: getPresence(workspaceId).includes(userId) } });
  };
  const leaveWorkspace = (socket: Socket, workspaceId: string) => {
    ...
    if (joined) announcePresence(workspaceId, socket.data.userId);
  };
  const unsubscribe = onAuthorizationChange(async (change) => {
  ...
  server.once('close', () => { unsubscribe(); if (io === socketServer) io = undefined; });
  ...
  socketServer.on('connection', (socket) => {
    ...
    const handle = (name: SocketRequest, action: (id: string, revision: number) => Promise<SocketAcknowledgement>) => {
      socket.on(name, (input: unknown, callback: unknown) => {
        ...
        void (async () => {
          ...
          const revision = authorizationRevision;
          try { await authenticateAccessToken(socket.data.token); }
    ...
    const stillAuthorized = (revision: number) => socket.connected && revision === authorizationRevision && Date.now() < socket.data.expiresAt;
    handle('workspace:join', async (id, revision) => {
      ...
      if (!alreadyJoined) announcePresence(id, socket.data.userId);
      return { ok: true };
    });
    ...
    handle('presence:list', async (id, revision) => {
      if (!await activeMembership(id, socket.data.userId) || !stillAuthorized(revision)) return denied();
      return { ok: true, users: getPresence(id) };
    });
