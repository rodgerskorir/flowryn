...
const createProject = async (agent: Session, workspaceId: string, name = 'Launch') => {
  const response = await agent.post(`/api/workspaces/${workspaceId}/projects`).send({ name, description: 'Ship the next release', color: '#336699' });
  expect(response.status).toBe(201);
  return response.body.project.id as string;
};
const createTask = async (agent: Session, workspaceId: string, projectId: string, values: Record<string, unknown> = {}) => {
  const response = await agent.post(`/api/workspaces/${workspaceId}/projects/${projectId}/tasks`).send({ title: 'Write brief', ...values });
  expect(response.status).toBe(201);
...
beforeEach(async () => {
  await Promise.all([
    ActivityModel.deleteMany({}), AuthSessionModel.deleteMany({}), TaskModel.deleteMany({}), ProjectModel.deleteMany({}),
    UserModel.deleteMany({}), WorkspaceModel.deleteMany({}), WorkspaceMemberModel.deleteMany({}),
  ]);
});
...
describe('projects, tasks, and activity', () => {
  it('allows owners to create, edit, archive projects and exposes task progress', async () => {
    ...
    const updated = await owner.patch(`/api/workspaces/${workspaceId}/projects/${projectId}`).send({ name: 'Launch v2' });
  ...
  it('supports task filters, status moves, ordering, and member assignment validation', async () => {
    ...
    await owner.post(`/api/workspaces/${workspaceId}/members`).send({ email: 'member@example.com', role: 'member' });
    await member.post('/api/auth/login').send({ email: 'member@example.com', password: 'password123' });
    const first = await createTask(member, workspaceId, projectId, { priority: 'urgent', assigneeId: memberUser!._id.toString() });
    await createTask(member, workspaceId, projectId, { priority: 'low' });
    const filtered = await member.get(`/api/workspaces/${workspaceId}/projects/${projectId}/tasks?priority=urgent`);
    expect(filtered.body.items).toHaveLength(1);
    expect(filtered.body.items[0].id).toBe(first.id);
    const moved = await member.patch(`/api/workspaces/${workspaceId}/tasks/${first.id}/status`).send({ status: 'in_progress' });
    expect(moved.status).toBe(200);
    expect(moved.body.task.status).toBe('in_progress');
    const reordered = await member.patch(`/api/workspaces/${workspaceId}/tasks/${first.id}/reorder`).send({ position: 12 });
    expect(reordered.body.task.position).toBe(12);
    const unassigned = await member.patch(`/api/workspaces/${workspaceId}/tasks/${first.id}/assignee`).send({ assigneeId: null });
    expect(unassigned.body.task.assigneeId).toBeNull();
    const invalidAssignee = await member.patch(`/api/workspaces/${workspaceId}/tasks/${first.id}/assignee`).send({ assigneeId: new mongoose.Types.ObjectId().toString() });
    expect(invalidAssignee.status).toBe(400);
    await UserModel.updateOne({ email: 'member@example.com' }, { status: 'suspended' });
    const suspendedAssignee = await owner.patch(`/api/workspaces/${workspaceId}/tasks/${first.id}/assignee`).send({ assigneeId: memberUser!._id.toString() });
  ...
  it('enforces project permissions and blocks cross-workspace IDs', async () => {
    ...
    const crossActivity = await ownerA.get(`/api/workspaces/${workspaceA}/projects/${projectA}/activity`);
    expect(crossActivity.status).toBe(200);
    expect(crossActivity.body.activities.every((entry: { workspaceId: string }) => entry.workspaceId === workspaceA)).toBe(true);
  });
});
