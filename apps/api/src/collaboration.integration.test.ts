...
const ack = (socket: Socket, event: SocketRequest, payload: unknown) =>
...
const fixture = async () => {
  ...
  const tokens = await Promise.all([
    issueTokens(owner.id),
    issueTokens(member.id),
    issueTokens(outsider.id),
  ]);
...
describe('collaboration boundaries', () => {
  it('accepts cookie sessions and rejects missing, invalid, expired and suspended credentials', async () => {
    ...
    const expired = jwt.sign(
      { sub: payload.sub, jti: payload.jti, type: 'access', exp: 1 },
      process.env.JWT_SECRET ?? 'flowryn-development-secret-change-me',
    );
    ...
    const cookieClient = connect(url, {
      extraHeaders: { Cookie: `accessToken=${f.tokens[0].accessToken}` },
      transports: ['websocket'],
      autoConnect: false,
    });
  ...
  it('delivers a private notification once, never to other workspace/project subscribers', async () => {
    ...
    const event = publishNotification(f.member.id, {
      workspaceId: f.workspace.id,
      projectId: f.project.id,
      actorId: f.owner.id,
      type: 'notification.created',
      payload: { title: 'Private', notificationId: f.project.id },
    });
  ...
  it('rejects malformed payloads for every handler and sanitizes async database failures', async () => {
    ...
    for (const event of [
      ...
      for (const value of ['bad-id', { $ne: null }, null, 123]) {
        expect(await ack(socket, event, value)).toMatchObject({
          ok: false,
          error: { code: 'INVALID_PAYLOAD' },
        });
      }
    }
    vi.spyOn(WorkspaceMemberModel, 'exists').mockRejectedValueOnce(
      new Error('secret database connection string'),
    );
    expect(await ack(socket, 'workspace:join', f.workspace.id)).toEqual({
      ok: false,
  ...
  it.each(['delete', 'disable'] as const)(
    'evicts every tab from workspace and project rooms on membership %s',
    async (operation) => {
      const f = await fixture();
      const tabs = await Promise.all([
        client(f.tokens[1].accessToken),
      ...
      await Promise.all(
        tabs.map(async (socket) => {
          await ack(socket, 'workspace:join', f.workspace.id);
          await ack(socket, 'project:join', f.project.id);
        }),
      );
      ...
      if (operation === 'delete')
        ...
        await WorkspaceMemberModel.updateOne(
          { userId: f.member.id, workspaceId: f.workspace.id },
          { disabled: true },
        );
      expect(await getPresence(f.workspace.id)).toEqual([]);
      for (const socket of tabs) {
        expect(
          [...gateway.sockets.sockets.get(socket.id!)!.rooms].some((room) =>
  ...
  it('does not let an in-flight room join restore membership after revocation', async () => {
    ...
    vi.spyOn(WorkspaceMemberModel, 'exists').mockImplementationOnce(() => {
      ...
      return new Promise((resolve) => {
        release = resolve;
      }) as ReturnType<typeof WorkspaceMemberModel.exists>;
    });
  ...
  it('enforces comment ownership/moderation, tenant isolation, and notification ownership/counts through REST', async () => {
    ...
    const taskResponse = await request(app)
      .post(`${base}/projects/${f.project.id}/tasks`)
      .set('Authorization', ownerHeader)
      .send({ title: 'Assigned task', assigneeId: f.member.id });
    ...
    expect((await request(app).delete(commentUrl).set('Authorization', memberHeader)).status).toBe(
      403,
    );
    expect((await request(app).get(comments).set('Authorization', outsiderHeader)).status).toBe(
      403,
    );
    expect(
      (
        await request(app)
          .get(comments.replace(f.workspace.id, f.other.id))
          .set('Authorization', outsiderHeader)
      ).status,
    ).toBe(404);
    expect(
      (await request(app).get(`${comments}?page=bad`).set('Authorization', ownerHeader)).status,
    ).toBe(400);
    expect(
      (
        await request(app)
    ...
    expect(
      (
        await request(app)
          .patch(`${base}/comments/${own.body.comment.id}`)
          .set('Authorization', memberHeader)
          .send({ body: 'My edit' })
      ).status,
    ).toBe(200);
    expect(
      (
        await request(app)
          .delete(`${base}/comments/${own.body.comment.id}`)
          .set('Authorization', ownerHeader)
      ).status,
    ).toBe(204);
    ...
    expect(
      (await request(app).get(`${base}/notifications`).set('Authorization', ownerHeader)).body
        .items,
    ).toEqual([]);
    ...
    expect(
      (
        await request(app)
          .patch(`${base}/notifications/${notificationId}/read`)
          .set('Authorization', ownerHeader)
      ).status,
    ).toBe(404);
    expect(
    ...
    expect(
      (
        await request(app)
          .get(`${base}/notifications?unread=false`)
          .set('Authorization', memberHeader)
      ).body.items,
    ).toHaveLength(2);
    expect(
    ...
    expect(
      (await request(app).get(`${base}/notifications`).set('Authorization', memberHeader)).status,
    ).toBe(403);
  });
});
...
it('delivers private notifications independently of selected projects, once per socket, including after reconnect', async () => {
  ...
  const recipient = await client(f.tokens[1].accessToken);
  const other = await client(f.tokens[0].accessToken);
  await ack(recipient, 'workspace:join', f.workspace.id);
  await ack(recipient, 'project:join', f.project.id);
  await ack(other, 'workspace:join', f.workspace.id);
  await ack(other, 'project:join', projectB.id);
  const received = vi.fn();
  const leaked = vi.fn();
  recipient.on('notification.created', received);
  other.on('notification.created', leaked);
  const send = (workspaceId = f.workspace.id) =>
    publishNotification(f.member.id, {
      workspaceId,
      projectId: projectB.id,
      actorId: f.owner.id,
      type: 'notification.created',
      payload: { title: 'Project B notification' },
    });
  let delivered = once(recipient, 'notification.created');
  send();
  await delivered;
  expect(received).toHaveBeenCalledTimes(1);
  await ack(recipient, 'project:join', projectB.id);
  delivered = once(recipient, 'notification.created');
  send();
  await delivered;
  send(f.other.id);
  await ack(recipient, 'presence:list', f.workspace.id);
  await ack(other, 'presence:list', f.workspace.id);
  expect(received).toHaveBeenCalledTimes(2);
  expect(leaked).not.toHaveBeenCalled();
  const gone = once(recipient, 'disconnect');
  gateway.sockets.sockets.get(recipient.id!)!.disconnect(true);
  await gone;
  const reconnected = await client(f.tokens[1].accessToken);
  await ack(reconnected, 'workspace:join', f.workspace.id);
  const restored = vi.fn();
  reconnected.on('notification.created', restored);
  delivered = once(reconnected, 'notification.created');
  send();
  await delivered;
  await ack(reconnected, 'presence:list', f.workspace.id);
  expect(restored).toHaveBeenCalledTimes(1);
  expect(received).toHaveBeenCalledTimes(2);
  const revoked = once(reconnected, 'disconnect');
  await revokeRefreshToken(f.tokens[1].refreshToken);
  await revoked;
  send();
  await ack(other, 'presence:list', f.workspace.id);
  expect(restored).toHaveBeenCalledTimes(1);
