...
Mongoose save and query update/delete hooks await local enforcement and Redis publication. Suspension invalidates stored sessions and disconnects sockets across instances; removed or disabled memberships evict workspace-qualified rooms on every instance. Logout and refresh rotation disconnect the revoked session. The internal control channel contains only a UUID event ID and a strict discriminated payload: kind/userId, plus workspaceId for membership or tokenId for session revocation. Receivers validate, deduplicate, and apply without republishing. Raw database writes and bulk writes bypass immediate hooks and are not supported for authorization mutations.
